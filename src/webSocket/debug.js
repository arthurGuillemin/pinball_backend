...
const obj = {
  ...
  boucle(objToLog, key, index) {
    ...
    if (Array.isArray(objToLog)) {
      ...
      index++;
      for (const x of objToLog) {
        leResultat += this.boucle(x, 'Set', index);
      }
      leResultat += tabulation(index - 1) + ']\n';
      ...
      leResultat += '\n';
      index++;
      for (let b in objToLog) {
        leResultat += this.boucle(objToLog[b], b, index);
      }
      leResultat += tabulation(index - 1) + '}\n';
    } else {
